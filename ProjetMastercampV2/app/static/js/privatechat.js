document.addEventListener('DOMContentLoaded', (event) => {
    ...
    function chatWith(user) {
        // Définir le destinataire de manière invisible
        document.getElementById('send_button').dataset.recipient = user;
    }

    socket.on('message', (data) => {
        if (data.username !== username) { // Vérifiez si le message n'est pas de l'expéditeur
    ...
    socket.on('private_message', (data) => {
        if (data.sender !== username) { // Vérifiez si le message privé n'est pas de l'expéditeur
            const messageElement = document.createElement('div');
            messageElement.classList.add('d-flex', 'justify-content-start', 'mb-4');
    ...
    document.getElementById('send_button').addEventListener('click', (event) => {
        ...
        if (message.trim()) {
            ...
            if (recipient) {
                socket.emit('private_message', { msg: message, recipient: recipient, sender: username });
            } else {
                socket.emit('message', { msg: message, username: username });
            }
    ...
    socket.on('audio_message', (data) => {
        if (data.username !== username) { // Vérifiez si le message audio n'est pas de l'expéditeur
